...
class ConditionalPluginModel(CMSPlugin):
    ...
    permitted_group = models.ForeignKey(Group, null=True, blank=True, on_delete=models.SET_NULL, verbose_name=_("Permitted group"))
    ...
    def __str__(self):
        if self.permitted_group is not None:
            return _('{mode}: {group}').format(
                mode=self.get_mode_display(),
                group=self.permitted_group.name,
            )
        else:
